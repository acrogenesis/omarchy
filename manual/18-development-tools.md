...
Existing installs automatically move compatible development containers and private named volumes into the rootless Docker store during update. The old rootful containers remain stopped as recovery copies. Containers that depend on elevated privileges, host directories, devices, custom runtimes, or custom networks stay untouched and stop the migration with an explanation so Omarchy never silently weakens or changes their access.
