...
Before the destination is first started, an error removes only the exact journal-owned destination container and restores the source. Journal-owned volumes remain available for ownership verification; a retry empties the verified volume before copying the current stopped source again, so a partial transfer or later source deletion cannot leave stale entries. Once a destination start has been attempted, automatic retry and source restoration stop for manual recovery because either side may contain new writes. Successful sources remain stopped with their restart policy disabled; they are retained in the rootful engine as recovery copies.
