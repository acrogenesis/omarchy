...
The migration restores the local user runtime environment for sudo-invoked upgrades and checks the user manager before changing Docker. If that manager is unavailable, log in as the target user and rerun the migration. Docker and its compatibility provider are replaced in one package transaction, preserving dependencies such as `once-bin`. Every installed Docker provider other than the Podman shim, including docker-git, receives workload preflight and transfer before replacement. Package repair leaves the compatibility package pending while any real Docker provider remains installed. Failed package-database queries abort rather than being treated as an absent engine.
